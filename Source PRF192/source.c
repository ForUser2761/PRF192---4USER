...
bool isCircle(double centerX, double centerY, double radius, double points[][2], int numPoints)
{
    for (int i = 0; i < numPoints; i++)
    {
        ...
        { // Sử dụng ngưỡng nhỏ để so sánh khoảng cách
            return false;
        }
    }
    return true;
...
/******************************************************************************* Các hàm với thư viện ctype.h */

/**
 * Kiểm tra một ký tự có phải là chữ cái (A–Z hoặc a–z)
 * @param c - ký tự cần kiểm tra
 * @return 1 nếu c là chữ cái, 0 nếu không
 */
int isAlpha(char c)
{
...
/**
 * Kiểm tra một ký tự có phải là chữ số (0–9)
 * @param c - ký tự cần kiểm tra
 * @return 1 nếu c là chữ số, 0 nếu không
 */
int isDigit(char c)
{
    return (c >= '0' && c <= '9');
...
/**
 * Kiểm tra một ký tự có phải là chữ cái hoặc chữ số
 * @param c - ký tự cần kiểm tra
 * @return 1 nếu c là chữ cái hoặc chữ số, 0 nếu không
 */
int isAlnum(char c)
{
...
/**
 * Kiểm tra một ký tự có phải là chữ thường (a–z)
 * @param c - ký tự cần kiểm tra
 * @return 1 nếu c là chữ thường, 0 nếu không
 */
int isLower(char c)
{
    return (c >= 'a' && c <= 'z');
...
/**
 * Kiểm tra một ký tự có phải là chữ hoa (A–Z)
 * @param c - ký tự cần kiểm tra
 * @return 1 nếu c là chữ hoa, 0 nếu không
 */
int isUpper(char c)
{
    return (c >= 'A' && c <= 'Z');
...
/**
 * Hàm kiểm tra xem ký tự có phải khoảng trắng không
 * Bao gồm: space, tab, newline, vertical tab, form feed, carriage return
 */
int isSpace(char c)
{
    return (c == ' ' || c == '\t' || c == '\n' || c == '\v' || c == '\f' || c == '\r');
...
/**
 * Hàm kiểm tra ký tự có phải là ký tự đặc biệt (punctuation)
...
/**
 * Hàm kiểm tra ký tự có phải là ký tự hệ thập lục phân (hexadecimal)
 * Bao gồm: 0–9, A–F, a–f
 */
int isXDigit(char c)
{
...
/**
 * Hàm kiểm tra ký tự có phải là ký tự điều khiển không
 * Các ký tự điều khiển có mã ASCII từ 0–31 và 127
 */
int isCntrl(char c)
{
...
/**
 * Hàm kiểm tra ký tự có thể in ra được (in ra màn hình)
 * Bao gồm: tất cả ký tự có mã ASCII từ 32 (space) đến 126 (~)
 */
int isPrint(char c)
{
    return (c >= 32 && c <= 126);
...
/**
 * Hàm kiểm tra ký tự là ký tự in được nhưng không phải khoảng trắng
 * (nghĩa là mã ASCII từ 33 đến 126)
 */
int isGraph(char c)
{
    return (c >= 33 && c <= 126);
...
/**
 * Hàm chuyển ký tự hoa (A–Z) thành thường (a–z)
 * Nếu không phải chữ hoa thì giữ nguyên ký tự
 */
char toLower(char c) {
    if (c >= 'A' && c <= 'Z') {
        return c + 32;  // Vì 'a' - 'A' = 32 theo bảng ASCII
    }
    return c;  // Không phải chữ hoa thì giữ nguyên
}

/**
 * Hàm chuyển ký tự thường (a–z) thành hoa (A–Z)
 * Nếu không phải chữ thường thì giữ nguyên ký tự
 */
char toUpper(char c) {
    if (c >= 'a' && c <= 'z') {
        return c - 32;  // Vì 'a' - 'A' = 32
    }
    return c;  // Không phải chữ thường thì giữ nguyên
}